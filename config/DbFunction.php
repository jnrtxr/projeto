...
class DbFunction{
	...
	function login($loginid,$password){
      ...
      if(!ctype_alpha($loginid) || !ctype_alpha($password)){
        ...
        echo "<script>alert('Senha ou Id incorretos!')</script>";
	...
	function create_course($cshort,$cfull,$cdate){
			...
			echo "<script>alert('Selecione o seu curso')</script>";
...
function create_subject($cshort,$cfull,$sub1,$sub2,$sub3){
			...
			echo "<script>alert('Selecione a sua disciplina')</script>";
			...
			if(false===$stmt){
				...
				$stmt->execute();
				echo "<script>alert('Disciplinas adicionadas)</script>";
...
function register($cshort,$cfull,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$ph,
   ...
   $query = "INSERT INTO `registration` (`course`, `subject`, `fname`, `mname`, `lname`, `gender`, `gname`, `ocp`,
			...
			$stmt->execute();
		   	echo "<script>alert('Aluno registrado com sucesso, o numero de registro dele é  $reg')</script>";
		 	//header('location:login.php');
...
function edit_course($cshort,$cfull,$udate,$id){
	...
	$stmt->execute();
    echo '<script>'; 
    echo 'alert("Curso Atualizado com sucesso!! ")'; 
    echo '</script>';
...
function edit_subject($sub1,$sub2,$sub3,$udate,$id){
	...
	$stmt->execute();
    echo '<script>'; 
    echo 'alert("Disciplinas atualizadas com sucesso")'; 
    echo '</script>';
...
function del_std($id){
 ...
 function del_subject($id){
    ...
    $stmt->bind_param('i',$id);
	$stmt->execute();
    echo "<script>alert('Disciplinas deletadas')</script>";
   // echo "<script>window.location.href='view-course.php'</script>";
}
